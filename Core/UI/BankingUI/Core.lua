...
function private.UpdateCurrentModule(frame)
	...
	if private.currentModule == "Warehousing" then
		...
		-- ahbot
		footerButtonsFrame:AddChild(TSMAPI_FOUR.UI.NewElement("Frame", "row3")
			:SetLayout("HORIZONTAL")
			:AddChild(TSMAPI_FOUR.UI.NewElement("ActionButton", "putBankBtn", "FOOTER_LEFT")
				-- :SetDisabled(TSM.IsWowClassic())
				:SetText("零存")
				:SetContext(TSM.Banking.Warehousing.PutBank)
				:SetScript("OnClick", private.GroupBtnOnClick)
			)
			:AddChild(TSMAPI_FOUR.UI.NewElement("ActionButton", "putBagBtn", "FOOTER_RIGHT")
				-- :SetDisabled(TSM.IsWowClassic())
				:SetText("整取")
				:SetContext(TSM.Banking.Warehousing.PutBag)
				:SetScript("OnClick", private.GroupBtnOnClick)
			)
		)
		-- ahbot
	...
	local function UpdateFrame(context)
		...
		if private.currentModule == "Warehousing" then
			...
			footerButtonsFrame:GetElement("restockBagsBtn")
				:SetDisabled(context.progress)
			footerButtonsFrame:GetElement("row3.putBankBtn")
--				:SetDisabled(context.progress or TSM.IsWowClassic())
				:SetDisabled(context.progress)
			footerButtonsFrame:GetElement("row3.putBagBtn")
--				:SetDisabled(context.progress or TSM.IsWowClassic())
				:SetDisabled(context.progress)
